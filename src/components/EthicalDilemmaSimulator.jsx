...
const EthicalDilemmaSimulator = ({ setView, currentUser }) => {
  ...
  // Load public dilemmas from Firestore
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "dilemmas"), (snapshot) => {
        ...
        setDilemmasList(fallbackDilemmas); 
    });

    return () => unsubscribe();
  }, []);

  // Function to load a dilemma AND check for a user's prior solution
  const loadDilemma = async (dilemma) => {
    ...
    if (solutionSnap.exists()) {
      // If they do, load their old solution and feedback, and LOCK the gamification points
      setUserResponse(solutionSnap.data().solution || '');
      setFeedback(solutionSnap.data().feedback || null);
      setIsNewSolution(false); 
    } else {
      // It's a fresh dilemma they haven't answered yet!
  ...
  // Run once on mount after dilemmasList is populated
  useEffect(() => {
  ...
  // Creates the PUBLIC dilemma doc
  ...
  // Writes the PRIVATE solution doc and handles AI Evaluation & Gamification
  const handleSolutionSubmit = async () => {
    ...
    try {
      ...
      // Save the AI feedback to that SAME private doc
      ...
      if (isNewSolution) {
          ...
          // Flip the lock so they can't spam points by resubmitting
          setIsNewSolution(false); 
      }
